...
def test_initialize_population(df_X_train):
    ...
    algo_object=GeneticOptimization(objective_function_topass)
    algo_object.initialize_population(df_X_train)
    assert algo_object.individuals.shape==(20,2)
...
def test__evaluate_fitness(df_model, df_X_train, df_y_train, df_X_valid, df_y_valid):
    ...
    algo_object._evaluate_fitness(df_model, df_X_train, df_y_train, df_X_valid, df_y_valid)
    assert len(algo_object.fitness_scores)==20
...
def test_fit(df_model, df_X_train, df_y_train, df_X_valid, df_y_valid):
    ...
    algo_object=GeneticOptimization(objective_function_topass,n_iteration=5,timeout=60*60)
